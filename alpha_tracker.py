...
JUNK_SIGNALS = ["fps", "tweak", "resume", " cv", "pdf", "youtube", "music", "optimizer", "adblock", "cookie"]
...
def get_market_news():
    ...
    # Removed strict 'hot' filter to ensure we always have news
...
def get_polymarket_real_signals():
    """Fixed loop and search logic for real Polymarket data."""
    url = "https://gamma-api.polymarket.com/markets?limit=20&active=true&order=volume&dir=desc"
    try:
        response = requests.get(url, timeout=10)
        ...
        for m in markets:
            ...
            # Verified list of keywords
            if any(k in text for k in ['ai', 'crypto', 'bitcoin', 'eth', 'open-ai', 'bot', 'layer']):
                slug = m.get('slug', '')
                ...
                if len(active_markets) >= 1: break
        return active_markets if active_markets else [{"title": "Global Crypto Prediction volume surge detected.", "source": "Polymarket", "type": "Expectation", "color": "text-indigo-400", "url": "https://polymarket.com/activity"}]
    except: return []
...
def run():
    all_projects = []
    seen_urls = set()
    print("--- AlphaTrace v4.6: Integrated Global Stream ---")
