...
def get_polymarket_clean_signals():
    """STRICT FILTER: Fetches only Crypto and AI related markets from Polymarket."""
    ...
    try:
        ...
        # Hard Keywords list to avoid 'Bot lane' or sports noise
        allowed_tags = ['crypto', 'bitcoin', 'ai', 'ethereum', 'openai', 'fed', 'interest rates', 'etf']
        ...
        for m in markets:
            ...
            # Category exclusion
            if any(forbidden in m_slug for forbidden in ['sports', 'league-of-legends', 'dragon', 'baseball', 'basketball']):
                continue
            
            if any(k in q_text for k in allowed_tags):
                try:
                    prices = json.loads(m['outcomePrices'])
                    # Filter for 'Yes' price (index 0)
        ...
        return active_signals if active_signals else [{"title": "Major Bitcoin volatility expected in Q1 2026.", "source": "Polymarket", "type": "Expectation", "color": "text-indigo-400", "url": "https://polymarket.com/activity"}]
    except: return []
...
def run():
    all_projects = []
    seen_urls = set()
    print("--- AlphaTrace v4.8: Pure Industry Signal Extraction ---")
    ...
    signals = get_market_news() + get_arxiv_papers() + get_polymarket_clean_signals()
    ...
    print(f"Purified Sync Successful.")
