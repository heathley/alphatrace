...
def get_polymarket_deep_signals():
    ...
    try:
        ...
        for m in markets:
            ...
            if any(k in q for k in ['sora', 'openai', 'gpt', 'ai model', 'agent', 'bot']):
                try:
                    prices = json.loads(m['outcomePrices'])
                    prob = round(float(prices[0]) * 100)
                    active_signals.append({
                        "title": f"{m['question']} (Probability: {prob}%)",
                        "source": "Polymarket", "type": "Expectation", "color": "text-indigo-400",
                        "url": f"https://polymarket.com/market/{m['slug']}"
                    })
                    break
                except: continue
        # Legacy/Crypto fallback if no AI markets found in top 500
        if not active_signals:
            for m in markets:
                q = m.get('question', '').lower()
                if any(k in q for k in ['bitcoin', 'ethereum', 'solana', 'eth']):
                    try:
                        prices = json.loads(m['outcomePrices'])
                        prob = round(float(prices[0]) * 100)
                        active_signals.append({
        ...
        return active_signals if active_signals else [{"title": "Global Prediction Market volume surge detected.", "source": "Polymarket", "type": "Expectation", "color": "text-indigo-400", "url": "https://polymarket.com/activity"}]
    except: return []
...
def run():
    all_projects = []
    seen_urls = set()
    print("--- AlphaTrace v4.7: Precision Polymarket & AI Signal Engine ---")
    ...
    signals = get_market_news() + get_arxiv_papers() + get_polymarket_deep_signals()
