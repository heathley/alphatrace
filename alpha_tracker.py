...
JUNK_SIGNALS = ["fps", "tweak", "resume", " cv", "pdf", "youtube", "music", "optimizer", "adblock", "cookie", "journaling", "password manager"]
...
def get_polymarket_real_signals():
    """Fetch real-time top-volume markets from Polymarket via Gamma API."""
    url = "https://gamma-api.polymarket.com/markets?limit=10&active=true&order=volume&dir=desc"
    try:
        response = requests.get(url, timeout=10)
        markets = response.json()
        active_markets = []
        # Target keywords: AI, GPT, Crypto, Bitcoin, Ethereum
        for m in markets:
            text = m.get('question', '').lower()
            if any(k in text for kw in ['ai', 'crypto', 'bitcoin', 'eth', 'open-ai', 'bot']):
                slug = m.get('slug', '')
                ...
                if len(active_markets) >= 1: break # Only need 1 prime signal
        return active_markets if active_markets else [{"title": "Crypto Sentiment rising on Prediction Markets.", "source": "Polymarket", "type": "Expectation", "color": "text-indigo-400", "url": "https://polymarket.com/activity"}]
    except: return []
...
def run():
    all_projects = []
    seen_urls = set()
    print("--- AlphaTrace v4.5: Polymarket Oracle Oracle Integration ---")
    ...
    # FINAL INTEGRATED SIGNAL FLOW
    signals = get_market_news() + get_arxiv_papers() + get_polymarket_real_signals()
    ...
    print(f"Full Autonomous Sync Successful.")
