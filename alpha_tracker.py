...
JUNK_SIGNALS = ["fps", "tweak", "resume", " cv", "pdf", "youtube", "music", "optimizer", "adblock", "cookie", "gaming"]
...
TAXONOMY = {
    ...
    "zk / Privacy / zkML": {
        ...
        "required_one": ["zk-", "zkp", "fhe", "snark", "stark", "prover", "verifier", "cryptography", "circuit"],
        "strict_crypto": True
    },
    "DePIN": {
...
def get_market_news():
    ...
    try:
        ...
        for post in data.get('results', [])[:2]:
            ...
            news.append({"title": post['title'], "source": source.split('.')[0].upper(), "type": "Market Alpha", "color": "text-orange-500", "url": post['url']})
        return news
    except: return []
...
def get_polymarket_industrial_signals():
    """ULTRA-STRICT FILTER: Rejects non-industry noise like GTA, War, or Celebs."""
    url = "https://gamma-api.polymarket.com/markets?limit=500&active=true&closed=false&order=volume&dir=desc"
    try:
        response = requests.get(url, timeout=15)
        markets = response.json()
        active_signals = []
        
        # Strictly crypto/AI whitelist
        industrial_whitelist = ['ethereum', 'bitcoin', 'openai', 'gpt', 'fed ', 'interest rates', 'solana', 'etf', 'sora']
        # Hard ban list
        noise_blacklist = ['gta', 'russia', 'ukraine', 'trump', 'biden', 'ceasefire', 'war', 'election', 'soccer', 'sports', 'oscar', 'celeb']
        
        for m in markets:
            q = m.get('question', '').lower()
            slug = m.get('slug', '').lower()
            
            # 1. Skip if noise blacklisted
            if any(b in q or b in slug for b in noise_blacklist):
                continue
            
            # 2. Check if industry whitelisted
            if any(w in q for w in industrial_whitelist):
                try:
                    prices = json.loads(m['outcomePrices'])
                    prob = round(float(prices[0]) * 100)
                    active_signals.append({
                        "title": f"{m['question']} (Probability: {prob}%)",
                        "source": "Polymarket",
                        "type": "Expectation",
                        "color": "text-indigo-400",
                        "url": f"https://polymarket.com/market/{m['slug']}"
                    })
                    if len(active_signals) >= 1: break
                except: continue
        
        # Fallback to high-confidence crypto ACTIVITY page if search fails
        return active_signals if active_signals else [{"title": "Bitcoin volatility remains high ahead of key FED decision.", "source": "Polymarket", "type": "Expectation", "color": "text-indigo-400", "url": "https://polymarket.com/activity"}]
    except: return []
...
def run():
    all_projects = []
    seen_urls = set()
    print("--- AlphaTrace v4.9: Industrial Signal Extraction ---")
    ...
    # REFINED SIGNAL MIX
    signals = get_market_news() + get_arxiv_papers() + get_polymarket_industrial_signals()
